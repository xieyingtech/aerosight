...
- 形成可在协议演示器和未来真实 Dock 上复用的 MQTT 摄取、命令、视频/遥测/传感实时数据和状态恢复路径；当前无设备阶段保持生产直连接入关闭。
...
- 不在缺少现场设备时开放 DJI Cloud API 直连创建入口或宣称实机控制、媒体和网络链路已经验收。
...
- ConnectorDefinition 注册表示平台理解其契约，不等于当前部署允许创建。Web 类型目录只列出同时完成实现、安全与当前环境验收的定义。
- 当前没有 Dock 2/3 实机，`dji.cloud-api` 不注册为可创建类型，也不在连接器页常驻渲染 LAN/Public、凭据和序列号表单；协议实现、Route Handler、数据库结构和历史实例保留，避免破坏兼容性。
- 当前连接器页采用已有实例列表和“新建连接器 → 选择类型”流程，首个可创建类型为已完成真实只读目录验收的 `dji.flighthub2`。
- 协议演示器与 fixture 结果记录为模拟/契约验证，不能提升生产类型可用性。未来开放直连时应以独立 Change 补齐 Dock 2/3、LAN/Public、媒体与控制现场验收。
...
5. 当前无设备阶段保持 `dji.cloud-api` 创建入口关闭，仅用 fixture/演示器验证协议、产品矩阵和失败关闭；连接器页只开放已验收的司空 2 类型。
6. 后续具备设备时另行在隔离项目完成 Dock 2/3 只读接入、局域网与公网网络、媒体及受控命令验收。
7. 完成对应产品的实机证据记录后，才可通过独立变更把直连定义加入可创建类型目录；未经验证的固件保持只读。
