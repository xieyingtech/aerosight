...
- `devices` 只保存实例身份、`device_type_id`、显示名称、当前状态、项目归属和可选的直接 Adapter 绑定；无人机、机场、机器人、摄像头和传感器使用同一张表，不存在 `device_components` 特殊分支。
...
### 2. DeviceType 绑定 Driver，Adapter 只承载项目连接配置
...
- `device_adapters` 是项目级 Driver 运行实例，保存网络 profile、凭据引用、租约和健康状态。同一个 DJI Adapter 可经机场网关管理多台 Device；设备可直接绑定 Adapter，也可通过 `gateway-for` 关系继承路由。
...
- Driver 升级只允许落入 DeviceType 的兼容范围；升级和 Adapter 重配不改变 Device ID。超出范围时先发布新 DeviceType 版本并显式迁移设备。

备选方案是让每台 Device 直接绑定任意 Driver；配置灵活但同一型号会产生不一致能力和协议选择，也难以统一升级，故 Driver 必须由 DeviceType 决定。另一个备选是让 DeviceType 保存项目凭据，会把全局类型目录和租户秘密耦合，故连接配置单独放在 Adapter 实例。

### 3. RBAC 使用设备范围 + capability action
...
### 4. DJI Driver 是 Go Worker 中的长连接边界

- 每个启用的 DJI Adapter 实例由 Worker 中的 DJI Driver 以租约单实例持有，建立 MQTT 5 会话并订阅该网关序列号下的状态、事件、请求和服务回复 Topic；多 Worker 通过数据库租约避免重复活跃连接。
...
### 5. 产品能力矩阵生成 DeviceType 并隔离 Dock 2/3 差异
...
### 7. 演示使用 MQTT Broker + MediaMTX，实时数据接口保持可替换
...
### 8. 协议演示器复用真实 Driver/Adapter，而不是写数据库

- 新演示器扮演 Dock/Pilot 侧客户端，连接同一个 MQTT Broker，发布 Dock 2 或 Dock 3 官方形状的拓扑、状态和事件，并消费平台服务 Topic 返回 ACK/NACK/延迟/超时。
- 视频使用两个可辨识的循环测试源代表相机设备，并模拟温度/环境传感器设备的时序采样；所有数据通过 DJI Driver/Adapter 的统一协议路径进入平台。
...
### 9. 算法服务使用通用 Provider/Definition/Run 三层模型
...
- 使用只增不删迁移：注册兼容 legacy/unknown Driver 和 DeviceType，为现有 `devices` 回填 `device_type_id`；将现有 Adapter 外部身份迁入或兼容视图到唯一绑定结构，并补充设备关系、有效能力、实时通道、细粒度 grant 与网络 profile。
- 复用现有命令、直播和算法表时优先增加版本、状态或 schema 字段，不复制第二套平行账本。旧的 `dji` adapter type 仍可读，但只有通过新 profile 验证后才能启用生产连接。
- 所有项目级新表直接含 `project_id`，设备关系使用 `(project_id, id)` 复合外键或等效事务校验。Driver 定义和系统 DeviceType 可为全局只读目录，项目自定义类型必须绑定团队/项目管理权限；Provider、实时订阅、命令和 Adapter 配置的每次读取都从已授权项目开始。
- 迁移不会自动为历史“违建算法”赋予法律结论；它被转换为普通 definition/category 与现有结果来源。

### 11. 控制安全以离散能力和命令账本为边界
