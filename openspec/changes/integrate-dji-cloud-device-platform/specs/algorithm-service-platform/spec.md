...
定义与业务类别无关的算法服务注册、版本化、运行和结果映射能力，使各种识别、检测、分割、跟踪、OCR 或自定义分析服务可以通过统一目录动态呈现在 UI 中。

## ADDED Requirements

### Requirement: 通用算法 Provider
系统 SHALL 允许项目管理员配置版本化算法 Provider，包括协议类型、服务地址、认证秘密引用、健康检查、超时、并发和网络策略；Provider MUST 不包含“违建”等固定业务类别语义。
...
### Requirement: 算法定义与版本独立于 Provider
系统 SHALL 允许在 Provider 上定义任意算法的名称、描述、能力类别、模型/进程标识、输入模式、参数模式、输出模式、结果映射和展示元数据；发布后版本 MUST 不可变，任务和运行 MUST 引用明确版本。

#### Scenario: 定义通用目标检测算法
- **GIVEN** Provider 可接收图像并返回目标集合
- **WHEN** 管理员完成输入输出映射测试并发布
- **THEN** 系统创建版本化算法定义，类别标签来自定义或服务元数据而非平台源码

#### Scenario: 更新生产模型
- **GIVEN** 已发布定义正被历史运行引用
- **WHEN** 管理员修改模型标识或映射
- **THEN** 系统创建新版本且不得改写历史运行的算法、参数和结果来源
...
每次算法调用 SHALL 创建不可变运行记录，包含项目、定义版本、输入资源版本、参数、幂等键、状态、尝试、外部任务标识、原始结果引用和标准化结果；运行状态至少覆盖排队、执行、成功、失败、取消和超时。

#### Scenario: 成功执行算法
- **GIVEN** 用户选择兼容输入和已发布算法版本
- **WHEN** Worker 调用 Provider 并收到有效结果
- **THEN** 系统保存运行来源、原始结果引用和标准化结果，并向项目发布状态变化
