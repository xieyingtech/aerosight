...
- **WHEN** 执行连接自检
- **THEN** 系统验证 TLS、服务健康、外部回调和推流入口，并在通过后允许启用 Adapter
...
系统 SHALL 提供不依赖真实 DJI 硬件的协议级演示器，可选择 Dock 2 或 Dock 3 场景，产生拓扑、遥测、命令 ACK/NACK/超时及直播源状态，且使用与实机 Adapter 相同的统一接入边界。
