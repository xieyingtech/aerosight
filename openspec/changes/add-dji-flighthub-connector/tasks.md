...
- [x] 5.1 在项目连接器类型目录注册“DJI 司空 2”，解释只读公有云同步边界；DJI Cloud API 直连不作为当前可创建类型
...
- [x] 5.6 添加组件与浏览器测试，覆盖无项目、单项目、多项目、Token 失效、项目被撤权、重复点击、离开页面清理和只读角色
- [x] 5.7 将连接器页改为已有实例列表与右上角“新建连接器”入口；点击后先选择类型，只有选择 `dji.flighthub2` 才按需打开司空向导，并移除常驻的 DJI Cloud API 直连表单
- [x] 5.8 添加列表、已断开实例、类型选择、关闭清理和权限测试，并用本地浏览器确认页面默认不出现其他接入方式或秘密配置字段

## 6. 文档、真实验收与交付

- [x] 6.1 更新环境变量和运维文档，说明区域 API 主机、出口 HTTPS、代理、同步周期、Token 最小权限/轮换、功能开关及司空与直连 Cloud API 的选择方式
- [x] 6.2 在司空测试组织完成项目发现、选择连接、首次同步、重复同步、手动同步、Token 撤销/更新、Worker 重启和断开验收，并保存脱敏证据
- [x] 6.3 使用真实司空目录中已有的 Dock 2/Matrice 3TD 脱敏证据与版本化 Dock 2/3 fixture 验证类型、拓扑和只读能力；确认无对应实机时不把任务、返航、机场调试、直播或其他现场设备接入声明为已验收能力
- [x] 6.4 运行迁移测试、`pnpm check`、`pnpm build`、Go 全量测试、安全验收和 OpenSpec strict validation，修复失败并记录最终结果
- [x] 6.5 按数据库、Web 握手、Worker runtime、UI 和文档边界复核差异，保留用户工作区中与当前 Change 无关的修改且不擅自提交
