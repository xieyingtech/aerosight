...
- [ ] 6.3 将官方直播启动、本地播放授权撤销和恢复接入 `live_streams` 状态机与 Worker 证据对账；不得调用未发布的停止接口或因租约过期误报远端停止，处理五分钟无观众、设备离线、凭据到期、响应未知及 Worker 重启，并用 fake clock 集成测试验证有证据时最终收敛、无证据时保持可解释未确认状态。
