...
Next.js 继续拥有同步数据库访问、团队成员授权和 UI；浏览器不持有组织 Token，也不直接调用 DJI。项目一级导航只表达平台通用领域，不加入司空或其他厂商专属入口：

- 总览、实时作业、任务、设备、案件和资产读取统一投影，不按连接器来源拆分页面。
- 总览与实时作业以地图为主工作画布，状态、选择、直播和操作进入右侧工作栏，避免纵向卡片挤压空间。
- 连接器页承载领域能力矩阵、同步水位、限流/权限/兼容诊断和只读重新探测，并提供当前连接器独有目录的扩展工作区入口。
- 航线目录、空间目录和建模目录作为连接器扩展工作区保留，不出现在全局项目侧边栏；可归一化的运行、轨迹、媒体、告警和资产仍进入通用页面。
- 受控操作仅渲染有效 capability，并展示风险、前置条件、审批和最终对账状态。
