...
设置 UI 只显示模式选择，不要求策略名称、规则版本 ID 或发布动作。保存后立即用于新触发的告警；数据库可继续使用不可变内部配置快照承载审计与历史运行引用，但该快照不构成用户可管理的版本。告警事务只写事实与 outbox；智能体失败、超时或模型不可用不能回滚或隐藏告警。每次自动化运行保存采用的配置快照、触发原因、输入事件、scope、产物和失败信息。管理员可按项目关闭所有自动 AI。
...
7. 在达到指标和安全演练通过后扩大项目范围；保留每项目设备下行、外部算法与自动 AI kill switch。
