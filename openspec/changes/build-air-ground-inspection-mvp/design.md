...
### 12. 告警后处理由版本化策略决定，AI 不是告警生成的同步依赖

`alert_automation_policies` 按项目/规则配置：
...
告警事务只写事实与 outbox；智能体失败、超时或模型不可用不能回滚或隐藏告警。每次自动化运行保存策略版本、触发原因、输入事件、scope、产物和失败信息。管理员可按项目关闭所有自动 AI。
