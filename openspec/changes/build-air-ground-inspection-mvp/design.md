...
- 用一次可部署的纵向实现打通模拟/DJI 无人机、实时遥测、巡检任务、直播/媒体、外部疑似违建识别、告警处置和时空回放。
...
- 形成以项目地图总览为默认入口、以统一时间线串联设备轨迹、任务、媒体、算法运行和告警的 UI 信息架构。

**Non-Goals:**

- 不在首期自研视频编解码、云端飞控、通用 SLAM、三维重建或模型训练框架。
- 不以大模型替代规则引擎、任务状态机、设备适配器或安全策略。
...
- Next.js 负责用户会话、项目授权、同步 CRUD、预签名媒体访问、任务/事件操作 API 和 Web UI。
- Go worker 负责设备适配器运行、遥测归一化、任务调度、命令投递、状态评估、规则处理、媒体派生调度、保留清理和报告生成。
...
- 任务：`task_versions`、`task_steps`、扩展 `task_runs`、`task_run_steps`、`device_commands`、`command_attempts`、`approvals`。
...
- 告警与治理：`event_rules`、`event_rule_versions`、`perception_events`、`event_feedback`、`alert_automation_policies`、`project_permissions`、`safety_policy_versions`、`idempotency_records`、`audit_events`、`outbox_events`、`generated_reports`。
...
`perception_events` 保存机器/规则事实，现有 `issues` 保存需要人员跟踪的工作项；事件可自动或手工创建一个 issue，并通过 `issue_links` 关联，避免把感知状态和人工工单状态混为一体。

### 6. 任务执行采用数据库持久化状态机和命令账本

- `task_versions` 固化任务定义；`task_runs.input_snapshot_json` 保存触发事件、选定版本和预检输入。
- 每个状态转换在事务中校验预期版本，更新当前状态，追加审计并写入 outbox；使用乐观版本号阻止并发覆盖。
- 调度器每次只推进一个可执行步骤。物理动作先创建 `device_commands`，通过安全策略后进入 `dispatchable`，收到匹配 ACK/NACK/结果后推进步骤。
- 可安全重试的命令复用同一幂等键；状态未知的非幂等命令进入 `paused` 等待人工处置。
- 紧急停止使用独立优先级队列和策略路径，但仍记录命令与确认；它可以撤销未投递普通命令，不能删除已发生记录。

选择理由：状态机和命令账本比通用工作流引擎更容易与现有表、权限和物理 ACK 建立事务关系。备选方案是首期引入 Temporal；其耐久工作流能力强，但会显著增加服务和开发模型，待流程数量与运行时长证明收益后再评估。
...
- 同一项目内按空间重叠、时间窗口和来源资产聚合为 `detection_group`，确定性规则再产生 `perception_event`，避免每帧一个告警。
...
- 项目级 layout 固定侧边栏：`总览`、`实时作业`、`任务`、`设备`、`告警事件`、`算法服务`、`智能体`、`数据资产`、`设置`。访问 `/projects/:projectId` 重定向或渲染 `总览`，而不是空白列表页。
- 总览由顶部项目状态与时间模式栏、中央 MapLibre 地图、右侧上下文详情抽屉、底部可折叠时间线组成；图层/筛选面板可覆盖在地图左侧，但不取代主导航。
- 地图按 layer 展示无人机、机巢、未来 ROS 机器人、实时/历史轨迹、任务区域与航线、媒体采集点、疑似违建 geometry 和告警。不同设备用不同图标和颜色，轨迹均读取统一 `Pose` / `Track` view model。
- 时间线不是单一日志列表，而是按设备轨迹、任务步骤、媒体、算法运行、检测和告警分轨。选择时间点或时间范围会同步地图位置/轨迹裁剪、右侧详情、媒体帧和事件列表；点击地图对象也反向定位时间线。
...
选择理由：地图承载“在哪里”，时间线承载“何时发生且前后如何关联”，两者共同作为时空数据的主索引。备选方案是为任务、视频和告警分别做孤立页面；会丢失跨域上下文，不采用。SSE 适合首期服务器单向推送；达到连接与扇出阈值后可迁移专用实时网关。
...
- 工具分为只读查询、写入草案和受保护调度三类。只读工具覆盖设备、任务、事件、资产和当前地图上下文；草案工具覆盖任务/报告/工单草案；受保护调度工具只能调用领域 API 的“请求执行”入口。
...
### 12. 告警后处理由项目当前模式决定，AI 不是告警生成的同步依赖

项目管理员只配置一个当前后处理模式：

- `manual`：仅生成告警，等待人工查看和处置。
- `agent-on-demand`：用户在告警页显式要求智能体总结、生成报告或拟定后续任务。
- `agent-auto-draft`：告警后异步生成报告/工单/任务草案，但不自动发布或执行。
- `follow-up-draft`：在前一模式上，根据规则生成机器人或人工复核草案；MVP 只保存草案，不要求真实 ROS 派单。

设置 UI 使用单行布局，左侧显示“自动 AI”，右侧只有模式选择；选择后立即保存，不要求独立启用开关、策略名称、规则版本 ID 或发布动作。`manual` 表示完全由人工处理，`agent-on-demand` 只允许用户显式请求，两者都不自动排队草案；仅 `agent-auto-draft` 和 `follow-up-draft` 自动运行。切换到非自动模式时，排队或运行中的自动草案 SHALL 被取消，原告警仍可人工处置。数据库可继续使用不可变内部配置快照承载审计与历史运行引用，但该快照不构成用户可管理的版本。告警事务只写事实与 outbox；智能体失败、超时或模型不可用不能回滚或隐藏告警。每次自动化运行保存采用的配置快照、触发原因、输入事件、scope、产物和失败信息。

选择理由：把“识别出了什么”和“接下来自动做什么”解耦，可以在尚未确定业务流程时先上线稳定告警链路，再逐步提高自动化等级。

### 13. 权限使用角色默认值 + 项目级显式授权

- 保留 `team_members.role`；owner/admin 获得项目管理默认权限，member 默认只读现有项目。
- `project_permissions` 为 member 增加 `event:handle`、`mission:operate`、`mission:approve`、`algorithm:manage`、`agent:use` 等显式权限，不改变其团队角色。
...
- 结构化日志统一携带 `request_id`、`event_id`、`project_id`、`device_id`、`task_run_id` 和 `command_id`，敏感字段先脱敏。
- 指标至少覆盖适配器连接、摄取速率/延迟/拒绝、outbox 积压、命令 ACK 延迟、任务状态、直播状态、算法调用延迟/错误/积压、规则处理延迟、SSE 连接、AI tool 鉴权拒绝和报告失败。
- MVP 目标：在基准负载下，遥测从接收到地图/时间线更新 P95 ≤ 2 秒，canonical detection 到告警可见 P95 ≤ 5 秒（不含外部算法耗时）；所有受保护调度具有审计记录，重复事件、算法 callback 和命令测试不产生重复副作用。
...
- [外部算法输出格式漂移或长时间不可用] → 版本化 mapping、契约测试、原始结果保全、熔断/退避和告警链路失败可见；不得将解析失败当作“无违建”。
...
4. 对内部项目启用对象存储、`http-json` 算法 provider、疑似违建 mapping、规则告警和 `manual` 自动化策略；验证 SSRF、秘密和 callback 安全测试。
5. 启用 AI SDK 只读/草案工具，再单独启用受保护调度；验证跨项目、撤权后排队和提示注入测试。
6. 接入 DJI 沙箱/测试组织，验证签名、设备身份映射、直播与命令能力，再对单个试点项目开启。
7. 在达到指标和安全演练通过后扩大项目范围；保留每项目设备下行、外部算法开关，并使用 `manual` 作为自动 AI 的关闭与回滚模式。
