...
- 仓库提供独立 Go 运维命令 `go -C apps/worker run ./cmd/rotate-credentials`，不注册为应用 `package.json` script。命令从当前进程的 `AUTH_SECRET` 读取旧密钥，默认通过无回显交互提示读取并确认新 Auth Secret；自动化环境可使用 `--new-secret-stdin` 从标准输入读取，密钥不得通过命令行参数传递。
- 轮换必须在停止 Web/worker 外部调用的维护窗口执行。命令先以只读 `--dry-run` 校验全部 credential envelope 均可由旧密钥解密；正式执行时获取数据库 advisory lock，在单个事务内锁定所有凭据行、解密、以新派生 key 和新随机 nonce 重新加密、立即回读验证，并仅在全部成功时提交。任何不支持版本、损坏密文、并发修改或验证失败均回滚全部写入。
- 成功输出只包含各资源类型的处理数量、新 key fingerprint 和后续“更新部署 `AUTH_SECRET` 并重启”的指引；输出、日志和审计不得包含旧/新 Auth Secret 或凭据明文。若旧 Auth Secret 已遗失，则无法解密或轮换旧密文，只能由管理员重新填写受影响凭据。

选择理由：这满足当前没有 KMS 的部署条件，同时避免数据库和备份直接暴露外部服务密钥。复用现有 `AUTH_SECRET` 不增加新的永久部署变量；原子轮换命令使计划内 Auth Secret 轮换无需重新填写凭据。代价是认证与凭据加密生命周期仍然耦合，旧 Auth Secret 遗失时无法恢复旧密文；未来可通过独立版本化主密钥或 KMS 消除该耦合。
...
- [`AUTH_SECRET` 轮换与数据库凭据密文不同步] → 维护窗口先运行 dry-run，再以旧/新密钥执行单事务批量重加密，成功后立即更新部署变量并重启；失败整体回滚。只有旧密钥遗失时才需要管理员重新填写凭据。
