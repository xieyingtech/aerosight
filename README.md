...
Both lifecycle modes also require OpenSSL and the `node:22-bookworm-slim` test image for an isolated HTTPS algorithm upstream. The helper's generated certificate is trusted inside the application's writable test tmpfs; TLS verification remains enabled. Provider configuration and a small input image are fixtures, while algorithm definition/run creation goes through the real API and outbox consumer. The test verifies signed asset bytes, the upstream's issued callback credentials, and completion/replay after restart without another upstream execution. The Node helper is test infrastructure in a separate container. The application image remains free of Node and source mounts. Result-file persistence across another restart and the complete device/mission/AI workflow remain separate acceptance checks.
