...
2. Copy the root `.env.example` to `.env.local`. Set `DATABASE_URL`, an `APP_SECRET` of at least 32 characters, and `CSRF_SECRET` (32 random bytes encoded as base64).
...
The container lifecycle and release-image tests also hold 24 concurrent project SSE connections while issuing 80 snapshot requests in batches of eight. They sample database connections against the default combined pool budget of 30, publish a new event after 32 seconds and require every stream to receive it, then verify all streams close on SIGTERM and database connections reach zero before restart. The same release-image drill covers API-created algorithm runs, signed callbacks and assets, AI tools and cancellation, simulated DJI telemetry, a resumed task with MQTT acknowledgement, and result files retained across process restarts.

`pnpm test:shutdown-recovery` separately exhausts the shutdown budget while an algorithm request is unfinished, then starts a fresh release-image process against the same database and object volume. It waits for the original outbox lease to expire without changing its timestamp and verifies one committed result. The synthetic upstream applies no effect on the interrupted first request; external providers still need their protocol's idempotency guarantees for responses lost after applying an effect.

The complete migration acceptance record is in `contracts/go-migration/final-acceptance.md`, with the original entrypoint mapping in `acceptance-matrix.md`. `pnpm test:legacy-contracts` checks frozen legacy responses and permission outputs; the actual database, browser and release-image tests are separate gates.
...
The integrated task case resumes a fixture paused task through the real API and verifies worker dispatch, a supported Dock 2 debug command and its MQTT acknowledgement, successful completion, and no duplicate service publication after restart. `mission-flow.json` and broker logs retain the result; the migrated application has no task-creation API. See `contracts/go-migration/single-binary-e2e.md` for the complete requirement/evidence mapping.

Lifecycle acceptance also builds the existing Go DJI simulator as a separate test executable and runs it against authenticated `eclipse-mosquitto:2.1.2-alpine`. API setup and connection checks enable the adapter; real Dock 2 topology/telemetry creates the device tree and appears in the project snapshot. After application restart, fresh telemetry and a higher lease epoch must appear without changing device identities. `device-flow.json` records this evidence. Media/NTP addresses in this fixture only support connection configuration; actual media playback has its separate acceptance runner. For standard gateway topics the runtime also subscribes to child `thing/product/+/state` and `thing/product/+/osd`; broker subscription ACLs must allow these filters. Every child message still requires a claimed gateway, and projection resolves device identity within its project/adapter.

Both lifecycle modes also require OpenSSL and the `node:22-bookworm-slim` test image for an isolated HTTPS algorithm upstream. The helper's generated certificate is trusted inside the application's writable test tmpfs; TLS verification remains enabled. Provider configuration and a small input image are fixtures, while algorithm definition/run creation goes through the real API and outbox consumer. The test verifies signed asset bytes, Go-issued callback credentials, and completion/replay after restart without another upstream execution. The Node helper is test infrastructure in a separate container. The application image remains free of Node and source mounts. A disposable named volume at `/var/lib/aerosight` stores actual input/result objects; another stop/restart after completion verifies result JSON/checksum, signed input download and callback replay. `object-persistence.json` records the checks, and cleanup removes the test volume.

The HTTPS helper also implements the AI Responses protocol. The lifecycle test creates an AI provider and session through the API, verifies all six read tools against the project database, checks retained evidence links and a sanitized upstream failure, and compares message history after restart. It holds another AI request at the upstream before SIGTERM, then verifies a single cancellation error, upstream connection closure, no fabricated assistant response and no replay after restart. The test client uses a 60-second deadline to distinguish service cancellation from client timeout. `ai-flow.json` records these checks. The upstream uses an internal Docker bridge on the reserved documentation subnet `203.0.113.0/24`, separate from the application's published-port network, so the production address policy and pinned TLS transport run without private-host exceptions.
...
Supply environment variables directly when running the executable. Use `AEROSIGHT_ENV=production`, an HTTPS `PUBLIC_ORIGIN`, and appropriate `HOST` and `PORT` values behind the deployment TLS endpoint. `HOST` defaults to `127.0.0.1` and `PORT` to `8080`; use an unbracketed IPv6 address such as `::` for `HOST`. In development, `PORT` belongs to Go and the Next.js port comes from `PUBLIC_ORIGIN` (default `http://localhost:3000`). Database, MQTT and MediaMTX remain external services. The separate callback listener belongs to the legacy worker maintenance command; unified `serve` handles callbacks on its HTTP port.

When upgrading, rename `AUTH_SECRET` to `APP_SECRET` and `CSRF_AUTH_KEY` to `CSRF_SECRET`, preserving both values exactly. No credential re-encryption is needed for a name-only change. Replace `HTTP_LISTEN_ADDRESS` with `HOST` and `PORT`, and rename `OBJECT_STORAGE_LOCAL_ROOT` to `DATA_DIR` while preserving its path and volume mount. The new executable reads only the new names. `DATA_DIR` remains the root of the existing object files; no files need to move. `PUBLIC_ORIGIN` is an origin without a path. Go has no standard application environment variable; `AEROSIGHT_ENV=development` enables development behavior, while other values use production protections.
...
The production environment file must provide `DATABASE_URL`, the existing `APP_SECRET`, `CSRF_SECRET`, and HTTPS `PUBLIC_ORIGIN`. Preserve the image's `HOST=0.0.0.0 PORT=8080` and `AEROSIGHT_ENV=production`; do not reuse the development example unchanged. Point database/MQTT/media addresses to hosts reachable from the container. Configure `CALLBACK_PUBLIC_BASE_URL` for the same public application endpoint when needed. A bind-mounted object directory must be writable by UID 10001; preserve object data across releases. Terminate TLS at the deployment endpoint and allow at least the configured `SHUTDOWN_TIMEOUT` (default 30 seconds) before forcibly stopping the container.

For an upgrade, retain the exact old Web/worker release and its configuration, including the original `APP_SECRET`. Take a verified database backup and an object-storage backup or versioned snapshot; both must describe the same cutover state. Stop the old Web and worker before running the new executable's `migrate`, then start `serve`. Do not allow both application versions to write concurrently. Check `/readyz`, login, tenant permissions, SSE, task processing and external callbacks before switching traffic. Users must sign in again because old Auth.js cookies are not Go sessions; account passwords and encrypted integration credentials retain their existing values and key semantics.
