...
The HTTPS helper also implements the AI Responses protocol. The lifecycle test creates an AI provider and session through the API, verifies all six read tools against the project database, checks retained evidence links and a sanitized upstream failure, and compares message history after restart. `ai-flow.json` records these checks. The upstream uses an internal Docker bridge on the reserved documentation subnet `203.0.113.0/24`, separate from the application's published-port network, so the production address policy and pinned TLS transport run without private-host exceptions. This test covers completed AI requests and history persistence; cancellation of an active AI request during shutdown remains a separate check.
