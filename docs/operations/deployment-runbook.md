...
本手册覆盖单区域试点部署。试点默认关闭设备命令、对象存储、外部算法和自动 AI；每个项目必须由管理员逐项启用。部署不等于允许飞行，现场负责人仍须完成法规、空域、设备和人员确认。
...
先保持 `AI_PROVIDER=disabled` 完成非 AI 纵向验收。需要 AI 时，从秘密管理器注入 key，设置 provider/model，重启 Web 并验证模拟或低风险只读工具。随后才开启项目 `automatic_ai_enabled`。该开关是自动 AI kill switch；关闭后已有告警仍可人工处置，但不得生成新的自动草案。智能体永远不能绕过预检、审批和命令账本直连设备。
...
5. `automatic_ai_enabled`：需要自动草案时单独开启；异常时最先关闭。
...
- 回滚后关闭 `device_commands_enabled`、`external_algorithms_enabled` 和 `automatic_ai_enabled`，保留只读总览；核对命令账本、未 ACK 命令、callback 和 outbox，再决定恢复。
...
4. 关闭项目设备命令和自动 AI，保留对象、遥测、审计和告警；创建异常报告并施加 retention hold。
...
- 功能开关按顺序启用，AI kill switch 与设备急停演练通过。
