...
发布时按部署手册记录不可变构建版本、迁移 ledger、数据库/对象目录备份和功能开关审批。先启用只读总览，再依次验证对象存储和算法；设备命令最后开启，自动 AI 独立审批。任何租户隔离、审计、重复命令、证据读取、readiness 或现场安全异常都应立即关闭设备命令、外部算法和自动 AI，并按回滚手册保留数据库与证据。
