...
2026-09-01（Asia/Shanghai）对候选基线 `d064286` 及当前 OpenSpec 实现工作区执行完整门禁，全部通过，无遗留代码、测试、迁移、构建或 OpenSpec 阻断项。工程门禁允许进入受控试点；实际开启设备命令前仍须完成部署手册中的现场法规、人员、空域、急停和双人审批检查。

## 门禁结果

| 门禁 | 结果 | 证据摘要 |
| --- | --- | --- |
| `pnpm check` | 通过 | TypeScript 无错误；294/294 Web 测试通过；全部 Go 包通过 |
| `pnpm test:all` | 通过 | 重跑 294 项 Web 与全部 Go 包；PostGIS 空库/现有/旧版/重复迁移通过；51/51 安全验收通过 |
| `pnpm benchmark:load` | 通过 | 遥测→地图/时间线 P95 44.03 ms ≤ 2,000 ms；detection→Task 条件→案件 P95 18.04 ms ≤ 5,000 ms；重复副作用为 0 |
| `pnpm drill:fresh-environment` | 通过 | 19 项配置/依赖契约、53 个迁移、Web/Worker 生产构建通过；AI 环境变量已移除 |
...
- 30 条 canonical detection，另有 30 次重复 callback；callback/detection/issue 均严格为 30，两个 Task 步骤各完成一次，共 60 个成功步骤。
- 遥测链路 P50 32.10 ms、P95 44.03 ms、最大 133.91 ms。
- detection → Task 条件 → 案件链路 P50 15.44 ms、P95 18.04 ms、最大 36.69 ms。
