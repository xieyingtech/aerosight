...
> 历史范围说明：本文记录早期 `add-dji-flighthub-connector` 的目录连接验收，不代表当前 89 接口平台仍“仅实现目录”，也不授权任何写操作。当前实现、真实只读证据和待完成现场项目以[司空专项运维与现场验收手册](./dji-flighthub-runbook.md)、[能力覆盖表](../../contracts/dji-flighthub/v2/CAPABILITY-COVERAGE.md)及当前 OpenSpec 验收证据为准。
...
本文验收时点仅开放司空 2 只读目录。后续版本已接入状态、任务、直播、地图和模型等读取及受治理的写入入口，但所有 action 仍默认关闭；获得明确现场授权并完成逐项 `field-write` 验收前，不得把代码接入、fixture 或真实 GET 结果解释为生产控制能力。
