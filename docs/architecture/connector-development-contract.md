...
- 连接器不得向全局项目侧边栏增加厂商专属入口。厂商目录、诊断和扩展工作区只能从对应连接器实例进入；设备、任务、实时流、地图和资产等通用事实必须进入平台既有页面。
- 连接器运行时随应用注册，是否同步和接受操作由连接器实例状态控制；不得再用厂商专属的部署总开关复制实例生命周期。高风险写操作仍使用独立能力、权限、审批和细粒度功能开关。
