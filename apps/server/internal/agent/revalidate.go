...
func RevalidateQueuedJob(ctx context.Context, tx *sql.Tx, jobID string, now time.Time) error {
	...
	err := tx.QueryRowContext(ctx, `
		select job.requested_by_user_id,job.team_id,job.project_id,job.session_id,job.required_permission,job.context_expires_at,
		       ...
		       coalesce(membership.role in('owner','admin'),false) or job.required_permission='project:view' or exists (
		         select 1 from project_permissions permission
		         where permission.project_id=job.project_id and permission.team_id=project.team_id
		         and permission.user_id=job.requested_by_user_id
		         and (permission.permission=job.required_permission
		              or (job.required_permission='issue:handle' and permission.permission='event:handle'))),
		       project.team_id,session.project_id,coalesce(session.started_by_user_id,0),session.status='open'
