...
SELECT device.project_id,device.device_type_id,device.status,capability.availability,capability.risk_level
FROM devices device JOIN device_capabilities capability ON capability.device_id=device.id AND capability.project_id=device.project_id
