...
select sqlc.arg(p3)::int as "teamId",member.role,
      ...
      exists(select 1 from connector_capability_snapshots capability
        where capability.project_id=adapter.project_id and capability.connector_instance_id=adapter.id
          ...
          and ((sqlc.arg(p9)='live-quality-set' and capability.device_model=device.device_model
                and capability.firmware_version=device.firmware_version)
            or (sqlc.arg(p9)<>'live-quality-set' and capability.device_model is null and capability.firmware_version is null))) as "capabilityFieldVerified",
     ...
     left join devices device on device.id=sqlc.arg(p5) and device.project_id=adapter.project_id
     ...
     left join connector_remote_resources target on target.id=sqlc.arg(p8) and target.project_id=adapter.project_id
...
-- name: FHLiveEnqueue :exec
insert into outbox_events(project_id,team_id,event_id,event_type,aggregate_type,aggregate_id,payload_json,max_attempts)
       values(sqlc.arg(p1),sqlc.arg(p2),sqlc.arg(p3),'flighthub.live_action.requested','connector_live_action_job',sqlc.arg(p4),sqlc.arg(p5),8)
       on conflict(event_id) do nothing;

-- name: FHLiveRead :many
SELECT to_jsonb(r) FROM (
select id::text,action_kind as action,capability_code as "capabilityCode",status,attempt_count as "attemptCount",
     ...
     from connector_live_action_jobs where id=sqlc.arg(p1)::uuid and project_id=sqlc.arg(p2) and connector_instance_id=sqlc.arg(p3)
) r;
