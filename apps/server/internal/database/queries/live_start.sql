...
SELECT device.status,device.adapter_id,adapter.adapter_type,device.device_type_id,
 ...
 profile.media_ingest_base_url,adapter.credential_envelope_json
...
CASE WHEN sqlc.arg(status)::text='live' THEN now() ELSE null END,sqlc.arg(lease_owner),now()+interval '45 seconds') RETURNING id;
