...
const listFlightHubConnections = `-- name: ListFlightHubConnections :many
...
func (q *Queries) ListFlightHubConnections(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, listFlightHubConnections, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listFlightHubIdentities = `-- name: ListFlightHubIdentities :many
select to_jsonb(row_data) from (
...
func (q *Queries) ListFlightHubIdentities(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, listFlightHubIdentities, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listFlightHubSyncRuns = `-- name: ListFlightHubSyncRuns :many
select to_jsonb(row_data) from (
...
func (q *Queries) ListFlightHubSyncRuns(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, listFlightHubSyncRuns, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
