...
const snapshotActiveTasks = `-- name: SnapshotActiveTasks :many
...
func (q *Queries) SnapshotActiveTasks(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotActiveTasks, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotAlgorithmRuns = `-- name: SnapshotAlgorithmRuns :many
...
func (q *Queries) SnapshotAlgorithmRuns(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotAlgorithmRuns, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotDeviceGrants = `-- name: SnapshotDeviceGrants :many
select to_jsonb(snapshot_row) as item from (
...
type SnapshotDeviceGrantsParams struct {
	ProjectID int32 `json:"project_id"`
	TeamID    int32 `json:"team_id"`
	UserID    int32 `json:"user_id"`
}

func (q *Queries) SnapshotDeviceGrants(ctx context.Context, arg SnapshotDeviceGrantsParams) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotDeviceGrants, arg.ProjectID, arg.TeamID, arg.UserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotDevices = `-- name: SnapshotDevices :many
SELECT to_jsonb(r) FROM (
...
func (q *Queries) SnapshotDevices(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotDevices, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotDiagnostics = `-- name: SnapshotDiagnostics :many
select to_jsonb(snapshot_row) as item from (
...
func (q *Queries) SnapshotDiagnostics(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotDiagnostics, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotIssues = `-- name: SnapshotIssues :many
select to_jsonb(snapshot_row) as item from (
...
func (q *Queries) SnapshotIssues(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotIssues, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotLiveStreams = `-- name: SnapshotLiveStreams :many
select to_jsonb(snapshot_row) as item from (
...
func (q *Queries) SnapshotLiveStreams(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotLiveStreams, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotMedia = `-- name: SnapshotMedia :many
select to_jsonb(snapshot_row) as item from (
...
func (q *Queries) SnapshotMedia(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotMedia, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const snapshotProjectScope = `-- name: SnapshotProjectScope :one
...
type SnapshotProjectScopeParams struct {
	ID     int32 `json:"id"`
	UserID int32 `json:"user_id"`
}

type SnapshotProjectScopeRow struct {
	ID               int32           `json:"id"`
	Name             string          `json:"name"`
	TeamId           int32           `json:"teamId"`
	Role             string          `json:"role"`
	DependencyHealth json.RawMessage `json:"dependencyHealth"`
}

func (q *Queries) SnapshotProjectScope(ctx context.Context, arg SnapshotProjectScopeParams) (SnapshotProjectScopeRow, error) {
	row := q.db.QueryRowContext(ctx, snapshotProjectScope, arg.ID, arg.UserID)
	var i SnapshotProjectScopeRow
	err := row.Scan(
		&i.ID,
		&i.Name,
		&i.TeamId,
...
const snapshotRealtimeChannels = `-- name: SnapshotRealtimeChannels :many
select to_jsonb(snapshot_row) as item from (
...
const snapshotTaskSteps = `-- name: SnapshotTaskSteps :many
SELECT to_jsonb(r) FROM (
...
func (q *Queries) SnapshotTaskSteps(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotTaskSteps, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) SnapshotTracks(ctx context.Context, projectID int32) ([]json.RawMessage, error) {
	rows, err := q.db.QueryContext(ctx, snapshotTracks, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
