...
func TestReportDraftLifecycleAndRollback(t *testing.T) {
	...
	res = f.request(t, "POST", path, "")
	data = decodedResponse(t, res)
	if res.StatusCode != 201 || data["completeness"] != "incomplete" || data["version"] != float64(1) || len(data["dataGaps"].([]any)) != 2 {
...
func TestReportAggregateIncludesAllEvidence(t *testing.T) {
	sources := reportSources{Run: gin.H{"id": float64(1), "status": "succeeded", "stateVersion": float64(3), "taskVersionId": "9", "taskVersion": float64(2), "deviceId": float64(4), "deviceUpdatedAt": "2026-09-06T00:00:00Z"}, Track: gin.H{"pointCount": float64(2), "endedAt": "2026-09-06T00:00:00Z"}, Steps: []gin.H{{"id": "5", "status": "succeeded"}}, Events: []gin.H{{"id": "event", "stateVersion": float64(1)}}, Feedback: []gin.H{{"id": "6", "issueId": float64(12), "reason": "人工确认", "createdAt": "2026-09-06T00:00:00Z"}}, Assets: []gin.H{{"id": float64(7), "version": float64(1), "checksum": "abc"}}}
	...
	want := []string{
		...
		"/projects/issues/detail/?issueId=12&projectId=1",
