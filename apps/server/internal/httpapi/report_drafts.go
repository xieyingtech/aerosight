...
func aggregateReport(pid, id int32, s reportSources) (gin.H, []gin.H, error) {
	...
	for _, rows := range [][]gin.H{s.Steps, s.Events, s.Feedback, s.Assets} {
	...
	return gin.H{"projectId": pid, "completeness": completeness, "dataGaps": gaps, "sections": gin.H{"taskRun": s.Run, "taskVersion": taskVersion, "device": device, "track": s.Track, "steps": s.Steps, "events": s.Events, "feedback": s.Feedback}, "conclusions": conclusions, "evidence": refs}, refs, nil
}
