...
func parseDeviceCommand(c *gin.Context) (deviceCommandInput, error) {
	...
	if v, ok := body["approvalRequestId"]; ok && v != nil {
		id, e := uuid.Parse(fhString(v))
		if e != nil || id.Version() < 1 || id.Version() > 5 || id.Variant() != uuid.RFC4122 {
			return out, bad
		}
...
func (s *Server) deviceCommandFailure(c *gin.Context, err error) {
	...
	if err != nil {
		candidate := err.Error()
		if strings.HasPrefix(candidate, "FLIGHTHUB_COMMAND_") || candidate == "FLIGHTHUB_CONNECTOR_NOT_CONNECTED" || candidate == "DEVICE_COMMAND_ROUTE_UNAVAILABLE" {
			code = candidate
		}
		switch candidate {
...
func (s *Server) writeDeviceCommand(ctx context.Context, w *database.WriteTx, uid, pid, teamID int32, input deviceCommandInput) (gin.H, error) {
	...
	routeRaw, err := q.FHCommandRoutes(ctx, sqlcgen.FHCommandRoutesParams{P1: pid, P2: input.DeviceID})
	routes, err := decodeFHRows(routeRaw, err)
	if err != nil {
		return nil, err
	}
	if len(routes) == 0 || len(routes) > 1 && routes[0]["priority"] == routes[1]["priority"] {
	...
	safetyContext := gin.H{"reason": input.Reason, "confirmation": input.Confirmation, "confirmedByUserId": uid, "confirmedAt": timestamp(time.Now()), "allowed": safety.Allowed, "confirmationRequired": safety.ConfirmationRequired, "activeTaskOverride": safety.ActiveTaskOverride}
	if routes[0]["connectorKey"] == "dji.flighthub2" {
		extra, err := fhCommandSafety(ctx, q, pid, teamID, input, target, routes[0])
		if err != nil {
			return nil, err
		}
		for k, v := range extra {
	...
	safetyRaw, err := json.Marshal(safetyContext)
	if err != nil {
		return nil, err
	}
