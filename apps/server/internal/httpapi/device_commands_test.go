...
func TestDeviceCommandSafetyIdempotencyAndAudit(t *testing.T) {
	...
	fixtureCommandRoute(t, f, pid, team, adapter, did)
...
func fixtureCommandRoute(t *testing.T, f *apiFixture, pid, team, adapter, did int) {
	t.Helper()
	if _, e := f.db.Exec("update device_adapters set connector_definition_id=(select id from connector_definitions where connector_key='simulator.memory' and version='1.0.0') where id=$1", adapter); e != nil {
		t.Fatal(e)
	}
	if _, e := f.db.Exec("with identity as (insert into device_external_identities(project_id,team_id,device_id,adapter_id,external_device_id,discovery_status) values($1,$2,$3,$4,'command-device','managed') returning id) insert into device_connector_bindings(project_id,team_id,device_id,connector_instance_id,external_identity_id) select $1,$2,$3,$4,id from identity", pid, team, did, adapter); e != nil {
		t.Fatal(e)
	}
}
