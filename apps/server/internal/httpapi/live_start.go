...
func (s *Server) startLiveStream(c *gin.Context) {
	...
	result, err := database.AuditedWrite(ctx, s.db, audit, s.authorizeWrite(uid, pid, access.TeamID, "mission:operate", false), func(w *database.WriteTx) (gin.H, error) {
		...
		if kind != "simulator" && kind != "dji" && !isFlightHub {
		...
		key := requested
		if errors.Is(e, sql.ErrNoRows) {
			if kind != "simulator" && !(isFlightHub && requested != "") {
		...
		lease := sql.NullString{String: "go:" + uuid.NewString(), Valid: !isFlightHub}
		sid, e := w.Queries.InsertLiveStartSession(ctx, sqlcgen.InsertLiveStartSessionParams{ProjectID: pid, TeamID: access.TeamID, DeviceID: did, AdapterID: target.AdapterID, StreamKey: key, ChannelID: channelID, SourceType: kind, Status: status, IngestRef: sql.NullString{String: ingest, Valid: !isFlightHub}, VendorRef: vendor, PlaybackRef: playback, ActorUserID: sql.NullInt32{Int32: uid, Valid: true}, LeaseOwner: lease})
		if e != nil {
			return nil, e
		}
		if kind == "dji" {
		...
		if isFlightHub {
			if _, e = w.Publish(ctx, database.ProjectEvent{ProjectID: pid, TeamID: access.TeamID, EventID: fmt.Sprintf("flighthub-live-start:%d", sid), EventType: "flighthub.live.start.requested", Payload: gin.H{"streamId": sid}}); e != nil {
				return nil, e
			}
		}
