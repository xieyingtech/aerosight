...
func TaskStepHandler(ctx context.Context, tx *sql.Tx, event outbox.Event) error {
	...
	err := tx.QueryRowContext(ctx, `select coalesce(run.created_by_user_id,task.created_by_user_id,project.created_by_user_id),run_step.status,
		coalesce(case when step.parameters_json->>'issueId' ~ '^[0-9]+$' then (step.parameters_json->>'issueId')::int end,
		  (select issue.id from issues issue where issue.project_id=run.project_id and issue.task_run_id=run.id order by issue.id desc limit 1),0),
		...
		event.ProjectID, event.TeamID, payload.TaskRunStepID, payload.TaskRunID).Scan(&userID, &status, &issueID, &copilotID)
