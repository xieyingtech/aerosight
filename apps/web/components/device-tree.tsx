...
function DeviceNode({ node, depth = 0 }: { node: DeviceTreeNode; depth?: number }) {
  ...
  return <li className="space-y-2">
    ...
    {!!node.children.length && <ul className="space-y-2">{node.children.map((child) => <DeviceNode depth={depth + 1} key={child.id} node={child} />)}</ul>}
  </li>;
}

export function DeviceTree({ nodes }: { nodes: DeviceTreeNode[] }) {
  return nodes.length ? <ul className="space-y-3">{nodes.map((node) => <DeviceNode key={node.id} node={node} />)}</ul>
