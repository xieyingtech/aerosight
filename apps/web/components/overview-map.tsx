...
import { realtimeDeviceHref } from "@/lib/overview-map-core";
...
export function OverviewMap({ snapshot }: { snapshot: ProjectSituationSnapshot }) {
  ...
  const realtimeHref = realtimeDeviceHref(snapshot.project.id, selection);
  return <section className="space-y-3" aria-label="项目态势地图">
    <div className="flex items-center justify-between">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">只读查看设备、任务、媒体与告警位置</p>
      </div>
    ...
    <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_280px]">
      ...
      <aside className="flex min-h-56 flex-col rounded-xl border bg-card p-4">
        {selection ? <>
          ...
          {realtimeHref ? <Link className="mt-4 inline-flex items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground" href={realtimeHref}>
            进入实时作业<ArrowRightIcon className="size-4" />
