...
const failedRun = {
  ...
  inputSnapshot: {
    ...
    definition: { definitionVersionId: 8, providerType: "http-json", modelOrProcess: "construction-v2", mappingVersion: "mapping-v4" },
    parameters: { threshold: 0.72 }, context: { capturedAt: "2026-08-27T01:00:00Z" },
    callback: { token: "must-never-render" }
  },
  canonicalResult: { mappingDiagnostics: ["detection[0] missing geometry", 9] },
...
test("failed run exposes versioned diagnostics, timing and raw result lineage without signed input secrets", () => {
  ...
  assert.equal(view.version.mappingVersion, "mapping-v4");
