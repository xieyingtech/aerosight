...
export async function listAlertAutomationSettings(projectId:number){
  ...
  const policies=(await query<Record<string,unknown>>(`select policy.id,policy.name,version.version,version.mode,version.status,version.event_rule_version_id as "eventRuleVersionId",version.published_at as "publishedAt" from alert_automation_policies policy left join alert_automation_policy_versions version on version.id=policy.current_published_version_id and version.project_id=policy.project_id where policy.project_id=$1 order by policy.name`,[projectId])).rows;
...
export async function publishAlertAutomationPolicy(projectId:number,rawInput:unknown,requestId?:string|null){const raw=rawInput as {name?:unknown;mode?:unknown;eventRuleVersionId?:unknown;config?:unknown};const input=alertAutomationPolicyInputSchema.parse({mode:raw.mode,eventRuleVersionId:raw.eventRuleVersionId,config:raw.config});const {user,access}=await requireAutomationAdmin(projectId);const name=typeof raw.name==="string"?raw.name.trim():"";if(!name)throw new Error("ALERT_AUTOMATION_POLICY_NAME_REQUIRED");return withAuditedProjectWrite({projectId,teamId:access.teamId,actorUserId:user.id,requestId:correlationId(requestId),action:"alert_automation.publish",resourceType:"alert_automation_policy",input:{name,...input},policyResult:{role:access.role}},async(client)=>{
  ...
  await client.query(`update alert_automation_policies set current_published_version_id=$3,updated_at=now() where id=$2 and project_id=$1`,[projectId,policy.id,version.id]);return {policyId:policy.id,versionId:version.id,version:next,mode:input.mode};
})}
