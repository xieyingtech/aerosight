...
export default async function TasksPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <Page title="任务编排"><div className="space-y-6">
    <section className="space-y-2"><h2 className="font-medium">任务模板</h2><DataTable columns={[{ key: "name", label: "名称", render: (task) => <Link className="font-medium text-primary hover:underline" href={`/projects/${projectId}/tasks/${String(task.id)}`}>{String(task.name)}</Link> }, { key: "triggerType", label: "触发类型" }, { key: "status", label: "状态" }]} items={items} /></section>
