...
function Workspace({project}:{project:Project}){
 ...
 return <APIStateView state={runsState}>{runs=><APIStateView state={catalogState}>{catalog=><APIStateView state={canManage?providersState:{...providersState,data:[]}}>{providers=>
  <Page title="算法运行" description="跟踪外部算法输入、Provider 模型来源、耗时、重试与原始结果证据"><div className="space-y-6">
    ...
    {canManage ? <><div><h2 className="text-lg font-semibold">算法服务配置</h2><p className="text-sm text-muted-foreground">连接凭据加密保存，读取和编辑时不会回显</p></div><AlgorithmProviderForm onChanged={providersState.reload} projectId={projectId} /><div className="space-y-3">{providers.length ? providers.map((provider) => <section className="space-y-2" key={String(provider.id)}><div className="flex flex-wrap items-center gap-2"><span className="font-medium">{provider.name}</span><Badge variant="outline">{provider.providerType}</Badge><Badge variant="outline">{provider.status}</Badge></div><AlgorithmProviderForm onChanged={providersState.reload} projectId={projectId} provider={provider} /></section>) : <p className="text-sm text-muted-foreground">尚未配置算法服务</p>}</div></> : <p className="text-sm text-muted-foreground">当前账号可查看运行，但不能配置服务或重试失败运行。</p>}
  </div></Page>
 }</APIStateView>}</APIStateView>}</APIStateView>;
}
