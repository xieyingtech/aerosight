...
import type { FlightHubFlightOperations } from "@/lib/dji-flighthub-flight-operations-core";
...
export default function FlightOperationsPage() {
 return <StaticAPIPage<FlightHubFlightOperations> endpoint={query=>{const id=positiveParam(query);return id?`/api/projects/${id}/flight-operations`:null;}}>{(operations,query)=><Workspace operations={operations} projectId={Number(query.get("projectId"))}/>}</StaticAPIPage>;
}
function Workspace({ operations, projectId }: { operations: FlightHubFlightOperations; projectId:number }) {
  return <Page
    title="航线与飞行任务"
