...
import { readFlightHubGeospatialActionJob, submitFlightHubGeospatialAction } from "@/lib/dji-flighthub-geospatial-actions";
...
function safeActionError(error: unknown) {
  ...
  if (message === "FLIGHTHUB_GEOSPATIAL_ACTION_NOT_FOUND") return { code: "action_not_found", status: 404 };
  if (message === "IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST") return { code: "idempotency_conflict", status: 409 };
  if (message === "FLIGHTHUB_GEOSPATIAL_ACTION_VERSION_CONFLICT") return { code: "version_conflict", status: 409 };
  if (message.startsWith("FLIGHTHUB_GEOSPATIAL_ACTION_")) return { code: message.toLowerCase(), status: 403 };
...
export async function POST(request: NextRequest,
  ...
  try {
    ...
    return NextResponse.json(await submitFlightHubGeospatialAction(projectId, { ...body, connectorInstanceId },
...
export async function GET(request: NextRequest,
  ...
  try {
    ...
    return NextResponse.json(await readFlightHubGeospatialActionJob(projectId, connectorInstanceId, jobId), { headers });
  } catch (error) {
    const safe = safeActionError(error);
    return NextResponse.json({ error: { code: safe.code } }, { status: safe.status, headers });
  }
