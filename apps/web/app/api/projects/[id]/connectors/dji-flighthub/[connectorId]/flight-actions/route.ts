...
import { readFlightHubActionJob, submitFlightHubAction } from "@/lib/dji-flighthub-flight-actions";
...
function safeActionError(error: unknown) {
  ...
  if (message === "FLIGHTHUB_ACTION_NOT_FOUND") return { code: "action_not_found", status: 404 };
  if (message === "IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_REQUEST") return { code: "idempotency_conflict", status: 409 };
  if (message.startsWith("FLIGHTHUB_ACTION_")) return { code: message.toLowerCase(), status: 403 };
...
export async function POST(
  ...
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body) || !Number.isInteger(projectId) || projectId <= 0
      || !Number.isInteger(connectorInstanceId) || connectorInstanceId <= 0) {
      throw new SyntaxError("invalid request");
    }
    const result = await submitFlightHubAction(projectId, { ...body, connectorInstanceId }, request.headers.get("x-request-id"));
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(await readFlightHubActionJob(projectId, connectorInstanceId, jobId), { headers });
  } catch (error) {
    const safe = safeActionError(error);
    return NextResponse.json({ error: { code: safe.code } }, { status: safe.status, headers });
  }
