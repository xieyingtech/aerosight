...
try {
  ...
  writeFileSync(resolve(output,'result.json'),JSON.stringify({passed:true,checks:['production embedded login hydration','secure session cookie','projects navigation','unapproved inline script blocked'],errors},null,2));
  console.log(`PASS: production browser login, hydration and CSP script rejection; evidence ${output}`);
} finally {
  await browser?.close();
