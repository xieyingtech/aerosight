...
import {
  disconnectFlightHubConnection,
  reconnectFlightHubConnection,
} from "@/lib/dji-flighthub-lifecycle";
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      await reconnectFlightHubConnection(Number(id), connectorId, request.headers.get("x-request-id")),
    ...
    const status = code === "access_denied" ? 403 : code === "connector_not_found" ? 404
      : code === "connector_not_disabled" ? 409 : 503;
