...
Go 版本为 1.26.1。直接安装和校验使用仓库 `go.mod` / `go.sum`；2026-09-08 最终 `go mod verify` 通过。sqlc 由 `scripts/sqlc.mjs` 精确锁定 v1.31.1，生成和漂移检查均已通过。前端沿用 `pnpm-lock.yaml`，使用 pnpm 10.33.0。
...
| gin-contrib/gzip | v1.2.7 | [静态文本压缩](https://github.com/gin-contrib/gzip)，已接入静态文本组，SSE/Range/私有 API 排除 |
...
| OpenAI Go SDK v3 | v3.56.0 | [Responses API](https://github.com/openai/openai-go)，已接入会话、工具调用和 provider 探测 |

版本锁定和模块校验不单独代表功能验收；最终压缩、AI、HTTP 与 sqlc 的行为证据见 final-acceptance.md。运行 Go 全套测试前需执行 `pnpm prepare:server`，复制原始迁移字节到嵌入目录；`pnpm test:worker` 已自动执行此步骤。
