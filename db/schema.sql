...
create table connector_device_admin_jobs (
  id uuid primary key default gen_random_uuid(),project_id integer not null,team_id integer not null,
  connector_instance_id bigint not null,device_id integer,requested_by_user_id integer not null,approval_request_id uuid not null,
  action_kind text not null,capability_code text not null,feature_flag text not null,idempotency_key text not null,
  request_digest text not null,request_envelope_json jsonb not null,status text not null default 'queued',attempt_count integer not null default 0,
  last_error_code text,result_json jsonb not null default '{}'::jsonb,result_envelope_json jsonb,attempted_at timestamptz,
  unknown_at timestamptz,completed_at timestamptz,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),
  ...
  constraint connector_device_admin_jobs_policy_valid check((action_kind='rtk-calibrate' and capability_code='device.rtk.calibrate' and feature_flag='flighthub.rtk.calibrate') or (action_kind='relay-pair' and capability_code='device.relay.pair' and feature_flag='flighthub.relay.pair') or (action_kind='active-project-update' and capability_code='device.active-project.update' and feature_flag='flighthub.device-migration') or (action_kind='sn-decrypt' and capability_code='security.sn.decrypt' and feature_flag='flighthub.sn-decrypt')),
...
--> statement-breakpoint
update driver_definitions set manifest_json=jsonb_set(manifest_json,'{capabilities}',manifest_json->'capabilities'||'[{"code":"rtk.calibrate","kind":"command","risk":"critical","inputSchema":{"type":"object"}},{"code":"relay.pair","kind":"command","risk":"critical","inputSchema":{"type":"object"}},{"code":"device.active-project.update","kind":"command","risk":"critical","inputSchema":{"type":"object"}}]'::jsonb,true),updated_at=now() where driver_key='dji.cloud' and version='1.0.0';
--> statement-breakpoint
update device_types set capability_profile_json=capability_profile_json||'{"rtk.calibrate":{"enabled":true},"device.active-project.update":{"enabled":true}}'::jsonb,updated_at=now() where type_key in('dji.dock2','dji.dock3');
--> statement-breakpoint
...
--> statement-breakpoint
update connector_definitions set manifest_json=jsonb_set(manifest_json,'{capabilities}',manifest_json->'capabilities'||'[{"code":"device.rtk.calibrate","kind":"action","risk":"critical","driverCapability":"rtk.calibrate","featureFlag":"flighthub.rtk.calibrate","defaultEnabled":false},{"code":"device.relay.pair","kind":"action","risk":"critical","driverCapability":"relay.pair","featureFlag":"flighthub.relay.pair","defaultEnabled":false},{"code":"device.active-project.update","kind":"action","risk":"critical","driverCapability":"device.active-project.update","featureFlag":"flighthub.device-migration","defaultEnabled":false},{"code":"security.sn.decrypt","kind":"action","risk":"high","featureFlag":"flighthub.sn-decrypt","defaultEnabled":false}]'::jsonb,true),updated_at=now() where connector_key='dji.flighthub2' and version='1.0.0';
--> statement-breakpoint
update driver_definitions
...
create table connector_model_jobs (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint connector_model_jobs_project_team_fk foreign key(project_id,team_id) references projects(id,team_id) on delete cascade,
  constraint connector_model_jobs_connector_project_fk foreign key(connector_instance_id,project_id) references device_adapters(id,project_id) on delete cascade,
  constraint connector_model_jobs_requester_member_fk foreign key(team_id,requested_by_user_id) references team_members(team_id,user_id) on delete restrict,
  constraint connector_model_jobs_action_valid check(action_kind in('traditional-create','open-start','open-stop')),
  constraint connector_model_jobs_status_valid check(status in('queued','reconciling','succeeded','failed','blocked')),
  constraint connector_model_jobs_idempotency_valid check(length(btrim(idempotency_key)) between 8 and 200 and idempotency_key=btrim(idempotency_key)),
  ...
  constraint connector_model_jobs_attempts_valid check(submit_attempt_count between 0 and 1 and reconciliation_count between 0 and 32),
  constraint connector_model_jobs_completion_valid check((status='succeeded')=(completed_at is not null)),
  constraint connector_model_jobs_project_idempotency_unique unique(project_id,connector_instance_id,action_kind,idempotency_key),
  constraint connector_model_jobs_id_project_unique unique(id,project_id)
);
--> statement-breakpoint
create index connector_model_jobs_pending_idx on connector_model_jobs(connector_instance_id,status,updated_at) where status in('queued','reconciling');
--> statement-breakpoint
create table connector_open_model_uploads (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint connector_open_model_uploads_project_team_fk foreign key(project_id,team_id) references projects(id,team_id) on delete cascade,
  constraint connector_open_model_uploads_connector_project_fk foreign key(connector_instance_id,project_id) references device_adapters(id,project_id) on delete cascade,
  constraint connector_open_model_uploads_requester_member_fk foreign key(team_id,requested_by_user_id) references team_members(team_id,user_id) on delete restrict,
  constraint connector_open_model_uploads_remote_project_fk foreign key(remote_resource_id,project_id) references connector_remote_resources(id,project_id) on delete set null (remote_resource_id),
  constraint connector_open_model_uploads_asset_project_fk foreign key(asset_id,project_id) references assets(id,project_id) on delete set null (asset_id),
  constraint connector_open_model_uploads_status_valid check(status in('requested','credential_ready','callback_pending','reconciling','succeeded','expired','failed','blocked')),
  constraint connector_open_model_uploads_idempotency_valid check(length(btrim(idempotency_key)) between 8 and 200 and idempotency_key=btrim(idempotency_key)),
  constraint connector_open_model_uploads_digest_valid check(request_digest ~ '^[a-f0-9]{64}$' and resource_uuid_digest ~ '^[a-f0-9]{64}$' and (callback_digest is null or callback_digest ~ '^[a-f0-9]{64}$')),
  constraint connector_open_model_uploads_envelope_object check(jsonb_typeof(request_envelope_json)='object' and (credential_envelope_json is null or jsonb_typeof(credential_envelope_json)='object') and (callback_envelope_json is null or jsonb_typeof(callback_envelope_json)='object')),
  constraint connector_open_model_uploads_callback_pair check((callback_envelope_json is null or callback_digest is not null) and (status not in('callback_pending','reconciling') or (callback_digest is not null and callback_envelope_json is not null))),
  constraint connector_open_model_uploads_attempts_valid check(callback_attempt_count between 0 and 1 and reconciliation_count between 0 and 16),
  constraint connector_open_model_uploads_completion_valid check((status='succeeded')=(completed_at is not null) and (status<>'succeeded' or (remote_resource_id is not null and asset_id is not null))),
  constraint connector_open_model_uploads_project_idempotency_unique unique(project_id,connector_instance_id,idempotency_key),
  constraint connector_open_model_uploads_id_project_unique unique(id,project_id)
);
--> statement-breakpoint
create index connector_open_model_uploads_pending_idx on connector_open_model_uploads(connector_instance_id,status,updated_at) where status not in('succeeded','expired','failed','blocked');
--> statement-breakpoint
